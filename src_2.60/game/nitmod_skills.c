...
static int NITMOD_ParseSkillRow( const char *text, int *row ) {
	char token[256];
	...
	for( level = 1; level < NITMOD_SKILL_LEVEL_COUNT; level++ ) {
		...
		token[size] = '\0';
		if( !NITMOD_ParseProtocolSigned( token, &row[level] ) ) return 0;
	}
	while( position < length && (text[position] == ' ' || text[position] == ',') ) position++;
...
int NITMOD_ParseSkillThresholds( nitmodSkillValue_t read, void *context,
	...
	for( skill = 0; skill < NITMOD_SKILL_COUNT; skill++ ) {
		if( !NITMOD_ParseSkillRow( read(context, keys[skill]), next.threshold[skill] ) ) return 0;
	}
	*output = next;
	return 1;
}
