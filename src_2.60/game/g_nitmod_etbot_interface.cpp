...
{
	char omniBotPath[MAX_OSPATH];
	if(G_NITMOD_LegacyCvarInteger("omnibot_enable", 0) == 0)
	{
		G_Printf("%s%s\n", S_COLOR_GREEN,
	...
	trap_Cvar_VariableStringBuffer("omnibot_path", omniBotPath, sizeof(omniBotPath));
	g_InterfaceFunctions = new ETInterface;
	/* Original Nitmod qagame 2.3.5 requests Omni-bot interface version 17. */
	eomnibot_error err = Omnibot_LoadLibrary(17,
		"omnibot_et", Omnibot_FixPath(omniBotPath));
...
{
	return IsOmnibotLoaded() ? qtrue : qfalse;
}
...
void Bot_Interface_ConsoleCommand()
{
	...
	if(IsOmnibotLoaded())
	{
		...
		Arguments args;
		for(int i = 0; i < trap_Argc() && i < Arguments::MaxArgs; ++i)
		{
