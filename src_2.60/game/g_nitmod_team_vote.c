...
static int TeamVote(gentity_t *ent, unsigned int index, char *arg, qboolean referee,
    ...
    if(trap_Argc() > 2) {
        G_NITMOD_TeamVoteUsage(ent,index,arg,referee);
        return G_INVALID;
    }
...
int G_NITMOD_ShuffleVote(gentity_t *ent, unsigned int index, char *arg,
    ...
    (void)arg2;
    return TeamVote(ent, index, arg, referee, vote_allow_shuffleteams.integer, Svcmd_ShuffleTeams_f);
}
int G_NITMOD_ShuffleNoRestartVote(gentity_t *ent, unsigned int index, char *arg,
    char *arg2, qboolean referee) {
    (void)arg2;
    return TeamVote(ent, index, arg, referee, vote_allow_shuffleteams_norestart.integer, G_shuffleTeams);
}
int G_NITMOD_SwapVote(gentity_t *ent, unsigned int index, char *arg,
    char *arg2, qboolean referee) {
    (void)arg2;
    return TeamVote(ent, index, arg, referee, vote_allow_swapteams.integer, G_swapTeams);
}
int G_NITMOD_SwapRestartVote(gentity_t *ent, unsigned int index, char *arg,
    char *arg2, qboolean referee) {
    (void)arg2;
    return TeamVote(ent, index, arg, referee, vote_allow_swapteamsrestart.integer, Svcmd_SwapTeams_f);
}
