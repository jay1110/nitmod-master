...
//----(SA)	end
	...
	if ( ! (ent->flags & FL_TEAMSLAVE ) )
	{
		if ( (ent->targetname && !ent->etxmapAutogeneratedName) || ent->takedamage)	// non touch/shoot doors
...
void SP_func_explosive (gentity_t *ent)
{
	...
	else if (ent->targetname && !ent->etxmapAutogeneratedName) {
...
void SP_func_brushmodel( gentity_t *ent ) {
	...
	if( ent->targetname && !ent->etxmapAutogeneratedName && level.numBrushModels < 128) {
